...
bool is_sec_check_hifi_tunningfile_crc32(char *buf)
{
	u32 *buf32 = NULL;
	u32 checksum;
	u32 checksum_base;
	...
	if (checksum != buf32[checksum_base]) {
		...
		set_bit(IS_CRC_ERROR_HIFI_TUNNING, &finfo->crc_error);
	} else {
		clear_bit(IS_CRC_ERROR_HIFI_TUNNING, &finfo->crc_error);
	}

	info("Camera: End checking CRC32 hifi tunning file\n");

	return !test_bit(IS_CRC_ERROR_HIFI_TUNNING, &finfo->crc_error);
}
#endif
#endif
...
//	old_mask = sys_umask(0);
	...
	if (force_caldata_dump) {
		...
		fd = ksys_open(name, O_WRONLY | O_CREAT | O_TRUNC | O_SYNC, 0664);
	}
	if (fd < 0) {
...
ssize_t read_data_rom_file(char *name, char *buf, size_t count, loff_t *pos)
{
	...
	fd = ksys_open(name, O_RDONLY, 0664);
	if (fd < 0) {
		if (-ENOENT == fd)
...
int is_sec_readcal_otprom(int rom_id)
{
	...
	ret = is_i2c_write(client, 0xA00, 0x04);
	if (unlikely(ret)) {
		err("failed to is_i2c_write (%d)\n", ret);
		ret = -EINVAL;
		goto exit;
	}
	is_i2c_write(client, 0xA02, 0x02);
	is_i2c_write(client, 0xA00, 0x01);

	ret = is_i2c_read(client, selected_page, 0xA12, 0x1);
	if (unlikely(ret)) {
		err("failed to is_i2c_read (%d)\n", ret);
		ret = -EINVAL;
		goto exit;
...
crc_retry:
	...
	is_i2c_read(client, buf, 0xA15, cal_size);
	...
	/* CRC check */
	ret = is_sec_check_cal_crc32(buf, rom_id);
...
void is_sec_get_hifi_tunningfile_addr(void)
{
	struct is_rom_info *finfo = NULL;
	is_sec_get_sysfs_finfo(&finfo, ROM_ID_REAR);
...
#ifdef CAMERA_MODULE_HIFI_TUNNINGF_DUMP
int is_sec_read_hifi_tunning_file(struct is_core *core)
{
	int ret = 0;
	loff_t pos = 0;
	char path[100];
...
crc_retry:
	...
	ret = is_spi_read(&core->spi0, fw_buf, finfo->hifi_tunning_start_addr,
		IS_MAX_HIFI_TUNNING_SIZE_LS);
	if (ret) {
		err("failed to is_spi_read (%d)\n", ret);
		ret = -EINVAL;
		goto exit;
	...
	/* CRC check */
	if (!is_sec_check_hifi_tunningfile_crc32(fw_buf) && (retry > 0)) {
		retry--;
		goto crc_retry;
	} else if (!retry) {
		ret = -EINVAL;
	...
	snprintf(path, sizeof(path), "%s%s", IS_FW_DUMP_PATH,
...
int is_sec_check_bin_files(struct is_core *core)
{
	...
	is_sec_get_hifi_tunningfile_addr();
#ifdef CAMERA_MODULE_REAR2_SETF_DUMP
...
#endif
#ifdef CAMERA_MODULE_HIFI_TUNNINGF_DUMP
			ret |= is_sec_read_hifi_tunning_file(core);
#endif
			if (ret < 0) {
...
#endif
#ifdef CAMERA_MODULE_HIFI_TUNNINGF_DUMP
					|| test_bit(IS_CRC_ERROR_HIFI_TUNNING, &finfo->crc_error)
#endif
					) {
					is_dumped_fw_loading_needed = false;
...
int is_sec_sensor_find_front_tof_mode_id(struct is_core *core, char *buf)
{
	...
	is_sec_get_sysfs_finfo(&finfo, FRONT_TOF_ROM_ID);

	if (finfo->cal_map_ver[3] >= FRONT_TOF_CHECK_MAP_VERSION) {
		char mode_id_list[256] = {0, };
		char mode_id_temp[10] = {0, };
		specific->front_tof_mode_id = *((int32_t*)&buf[finfo->rom_tof_cal_mode_id_addr]);
		info("front_tof_mode_id: %x\n", specific->front_tof_mode_id);
	} else {
		for (i = 0; i < finfo->rom_tof_cal_mode_id_addr_len; i++) {
			specific->front_tof_mode_id[i] = FRONT_TOF_DEFAULT_UID;
		}
		specific->front_tof_mode_id = FRONT_TOF_DEFAULT_UID;
		info("front_tof_mode_id: 0x%x, use default 0x%x", *((int32_t*)&buf[finfo->rom_tof_cal_mode_id_addr]),
													specific->front_tof_mode_id);
	}
#endif
	return 0;
...
int is_sec_fw_find(struct is_core *core)
{
	...
	switch (rear_id) {
	case SENSOR_NAME_S5K2LD:
		snprintf(finfo->load_setfile_name, sizeof(IS_2LD_SETF), "%s", IS_2LD_SETF);
		snprintf(finfo->load_rta_fw_name, sizeof(IS_RTA), "%s", IS_RTA);
		break;
	case SENSOR_NAME_SAK2L4:
		snprintf(finfo->load_setfile_name, sizeof(IS_2L4_SETF), "%s", IS_2L4_SETF);
		snprintf(finfo->load_rta_fw_name, sizeof(IS_RTA), "%s", IS_RTA);
		break;
	default:
		snprintf(finfo->load_setfile_name, sizeof(IS_2LD_SETF), "%s", IS_2LD_SETF);
...
#if defined(IS_TUNNING_HIFILLS)
	snprintf(finfo->load_tunning_hifills_name, sizeof(IS_TUNNING_HIFILLS),
		"%s", IS_TUNNING_HIFILLS);
#endif
